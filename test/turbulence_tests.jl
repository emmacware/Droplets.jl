...
@testset "implicit_diffuse!" begin
    ...
    @testset "surface flux is applied" begin
        ...
        K = fill(FT(5.0), nz)
        implicit_diffuse!(ϕ_noflux, K, dt, dz, nz)
        implicit_diffuse!(ϕ_sflux,  K, dt, dz, nz; sfc_flux=FT(10.0))
...
# ──────────────────��──────────────────────────────────────────────────────────
# 3.  mixing_length_deardorff!
# ─────────────────────────────────────────────────────────────────────────────

@testset "mixing_length_deardorff!" begin
    ...
    @testset "all lengths are positive" begin
        grid = make_test_grid(nz, dz)
        l = zeros(FT, nz)
        mixing_length_deardorff!(l, grid, l_inf, constants)
        @test all(l .> 0.0)
    end

    @testset "floor is enforced (l ≥ 1e-3)" begin
    ...
    @testset "neutral/unstable → l == dz (capped at grid spacing)" begin
        # Unstable → N² ≤ 0 branch → l = dz (floor-clipped if dz < 1e-3)
        θ_unstable = FT[295.0 - 0.1*k for k in 1:nz]
        grid = make_test_grid(nz, dz; θ_profile=θ_unstable)
        l = zeros(FT, nz)
        mixing_length_deardorff!(l, grid, l_inf, constants)
        @test all(l .== max(dz, FT(1e-3)))
    end

    @testset "stable → l ≤ dz (Deardorff limiter active)" begin
        θ_stable = FT[288.0 + 0.01*k for k in 1:nz]
        e_small  = fill(FT(1e-4), nz)
        grid = make_test_grid(nz, dz; θ_profile=θ_stable, e=e_small)
        l = zeros(FT, nz)
        mixing_length_deardorff!(l, grid, l_inf, constants)
        @test all(l .<= dz .+ 1e-12)
    end
    ...
    @testset "larger TKE → larger mixing length (stable regime)" begin
        ...
        l_lo = zeros(FT, nz);  mixing_length_deardorff!(l_lo, g_lo, l_inf, constants)
        l_hi = zeros(FT, nz);  mixing_length_deardorff!(l_hi, g_hi, l_inf, constants)
...
# ─────────────────────────────────────────────────────────────────────────────
# 4.  no_prog_tke_timestep!  (integration)
# ─────────────────────────────────────────────────────────────────────────────

@testset "no_prog_tke_timestep!" begin
    ...
    @testset "TKE floor is enforced after timestep" begin
        ...
        @test all(grid.states.e .>= tke.e_min)
    end

    @testset "surface TKE BC is applied (e[1] ≥ u_star² * c_ε)" begin
        grid = make_test_grid(nz, dz)
        no_prog_tke_timestep!(grid, tke, constants, dt)
        e_sfc_min = tke.u_star^2 * tke.c_ε
        @test grid.states.e[1] >= e_sfc_min
    end
    ...
    @testset "θ and qv stay finite after no_prog_tke_timestep!" begin
        grid = make_test_grid(nz, dz)
        no_prog_tke_timestep!(grid, tke, constants, dt)
